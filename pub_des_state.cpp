...
void DesStatePublisher::pub_next_state() {
    ...
    if (e_stop_trigger_) {
	ROS_INFO("Im in e stop trigger loop");
    ...
    if (e_stop_reset_) {
	    ...
	    // ctt14, small edit but ensures robot continues pursuing subgoals after obstacle in front of it is removed.
	    motion_mode_ = DONE_W_SUBGOAL;
        }
        //OK...want to resume motion from e-stopped mode;
        else {
	    ROS_INFO("Im in else statment to resume motion from before e stop");
    ...
    switch (motion_mode_) {
		...
		ROS_INFO("Publishing seg end state");
